...
func (h *Handler) ExamStart(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, fmt.Sprintf("/exam/%d/take", attempt.ID), http.StatusSeeOther)
}
...
func (h *Handler) ExamSubmit(w http.ResponseWriter, r *http.Request) {
	...
	// Get the questions for this attempt to record answers
	questions, err := h.Questions.GetQuestions(attempt.SubjectID, attempt.TotalQuestions)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	for _, q := range questions {
		...
		values := r.Form[key]
		if len(values) == 0 {
			h.Attempts.RecordAnswer(attemptID, q.ID, nil)
			continue
		}
		var answerIDs []int64
		...
		h.Attempts.RecordAnswers(attemptID, q.ID, answerIDs)
	}
